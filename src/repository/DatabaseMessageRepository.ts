...
async function vFindManyMessageByFulltext(input: string | undefined) {
  ...
  return prisma.$queryRaw<MessageFulltext[]>`
    SELECT
        rank
      , messages.team_id
      , teams.team_name
    ...
    WHERE
      v_messages.text MATCH ${fulltext}
    ORDER BY
      rank ASC
      , ts DESC`;
}
